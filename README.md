...
`MCP_AUTH_TOKEN` is generated by Render on first deploy; copy it from the
dashboard's Environment tab.

```powershell
claude mcp add --transport http helios-expenses `
...
uv run tests/http_test.py https://<your-service>.onrender.com/mcp <token>
```
...
- **Auth.** Without `MCP_AUTH_TOKEN` the endpoint is open to anyone who finds the
  URL; the server logs a warning at boot. The token is enforced on `/mcp` only —
  `/` and `/healthz` stay public for Render's health check.
...
app.py         ASGI app for hosted HTTP deployment (auth, host allowlist, /healthz)
